...
class WS {
    ...
    switch (data.op) {
      ...
      case 0: {
        try {
          ...
          this.client.emit(
            "debug",
            `${this.libName} ${this.shardCatastrophic} @ ${this.time()} => ERROR at ${data.t}: ${error}`,
          );
      ...
      case 1: {
        this.client.emit(
          "debug",
          `${this.libName} ${this.shardNorminal} @ ${this.time()} => Gateway requested heartbeat`,
        );
      ...
      case 7: {
        this.client.emit(
          "debug",
          `${this.libName} ${this.shardWarning} @ ${this.time()} => Received reconnect`,
        );
      ...
      case 9: {
        this.client.emit(
          "debug",
          `${this.libName} ${this.shardWarning} @ ${this.time()} => INVALID SESSION`,
        );
      ...
      case 11: {
        ...
        this.client.emit(
          "debug",
          `${this.libName} ${this.shardNorminal} @ ${this.time()} => Heartbeat acknowledged`,
        );
  ...
  heartbeat(response = false) {
    ...
    this.client.emit(
      "debug",
      `${this.libName} ${this.shardNorminal} @ ${this.time()} => Sending heartbeat...`,
    );
    ...
    if (response != true)
      setTimeout(() => {
        if (this.waitingForHeartbeatACK == true && this.resuming != true) {
          this.client.emit(
            "debug",
            `${this.libName} ${this.shardCatastrophic} @ ${this.time()} => Heartbeat ACK not received`,
          );
  ...
  identify() {
    this.client.emit(
      "debug",
      `${this.libName} ${this.shardNorminal} @ ${this.time()} => IDENTIFY with TOKEN: "${this.token}", SHARD: "${this.shard}" and INTENTS: "${this.intents}"`,
    );
  ...
  reconnect() {
    this.client.emit(
      "debug",
      `${this.libName} ${this.shardWarning} @ ${this.time()} => Attempting reconnect...`,
    );
    ...
    this.client.emit(
      "debug",
      `${this.libName} ${this.shardWarning} @ ${this.time()} => Shard reconnecting`,
    );
  }
  ...
  resume() {
    ...
    this.client.emit(
      "debug",
      `${this.libName} ${this.shardWarning} @ ${this.time()} => RESUMING with token ${this.token}, session id ${this.sessionId} and sequence ${this.s}`,
    );
  ...
  addListeners() {
    this.client.emit(
      "debug",
      `${this.libName} ${this.shardWarning} @ ${this.time()} => Adding websocket listeners`,
    );
    ...
    this.ws.once("open", () => {
      this.client.emit(
        "debug",
        `${this.libName} ${this.shardNorminal} @ ${this.time()} => Websocket opened`,
      );
    ...
    this.ws.once("close", (data) => {
      this.client.emit(
        "debug",
        `${this.libName} ${data < 2000 ? this.shardNorminal : this.shardCatastrophic} @ ${this.time()} => Websocket closed with code ${data}`,
      );
      ...
      if (this.retries <= 5)
        setTimeout(() => {
          this.client.emit(
            "debug",
            `${this.libName} ${this.shardWarning} @ ${this.time()} => Attempt ${this.retries} at re-opening websocket`,
          );
          ...
          this.monitorOpened = setTimeout(() => {
            this.client.emit(
              "debug",
              `${this.libName} ${this.shardWarning} @ ${this.time()} => Attempt ${this.retries} failed to re-open websocket, shutting down websocket with code ${data}`,
            );
    ...
    this.ws.on("error", (data) => {
      this.client.emit(
        "debug",
        `${this.libName} ${this.shardCatastrophic} @ ${this.time()} => ${data?.stack?.toString()}`,
      );
  ...
  async shutDownWebsocket(code = 1000) {
    this.client.emit(
      "debug",
      `${this.libName} ${this.shardWarning} @ ${this.time()} => Closing websocket...`,
    );
    ...
    this.terminateSocketTimeout = setTimeout(() => {
      this.client.emit(
        "debug",
        `${this.libName} ${this.shardCatastrophic} @ ${this.time()} => Terminating websocket`,
      );
      this.ws.terminate();
      setTimeout(() => {
