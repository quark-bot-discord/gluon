...
class MessageReactionManager {
    ...
    constructor(client, guild, existingReactions = {}) {
        ...
        for (const message_reaction in existingReactions)
            this.cache[message_reaction] = new Reaction(this.client, existingReactions[message_reaction], this.guild.id.toString());
    ...
    removeReaction(user_id, emoji) {
        ...
        if (this.cache[emoji]) {
            ...
            this.cache[emoji]._reacted = this.cache[emoji]._reacted.filter(userId => userId != user_id);

            if (this.cache[emoji]._reacted.length == 0)
                delete this.cache[emoji];
