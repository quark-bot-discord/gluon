...
class Guild {
  ...
  constructor(client, data, nocache = false) {
    ...
    if (typeof data.premium_subscription_count == "number")
      ...
      existing &&
      existing.premium_subscription_count
    )
    ...
    if (
      ...
      (existing && typeof existing.mfa_level == "number")
    ) {
      const mfaLevel =
        typeof data.mfa_level == "number" ? data.mfa_level : existing.mfa_level;
      switch (mfaLevel) {
        case 0:
          // none
    ...
    if (
      ...
      (existing && typeof existing.verification_level == "number")
    ) {
    ...
    if (
      ...
      (existing && typeof existing.default_message_notifications == "number")
    ) {
      const defaultMessageNotifications =
        typeof data.default_message_notifications == "number"
          ? data.default_message_notifications
          : existing.default_message_notifications;
    ...
    if (
      ...
      (existing && typeof existing.explicit_content_filter == "number")
    ) {
      const explicitContentFilter =
        typeof data.explicit_content_filter == "number"
          ? data.explicit_content_filter
          : existing.explicit_content_filter;
    ...
    if (
      ...
      (existing && typeof existing.nsfw_level == "number")
    ) {
      const nsfwLevel =
        typeof data.nsfw_level == "number"
          ? data.nsfw_level
    ...
    if (
      ...
      (existing && typeof existing.premium_tier == "number")
    ) {
      const premiumTier =
    ...
    if (
      ...
      existing &&
      typeof existing.premium_progress_bar_enabled == "boolean" &&
      existing.premium_progress_bar_enabled == true
    )
      this._attributes |= 0b1 << 26;
  ...
  async ban(user_id, { reason, days } = {}) {
    if (!checkPermission(await this.me(), PERMISSIONS.BAN_MEMBERS))
      throw {
        status: 403,
  ...
  async kick(user_id, { reason } = {}) {
    if (!checkPermission(await this.me(), PERMISSIONS.KICK_MEMBERS))
      throw {
        status: 403,
        error: "The bot does not have the KICK_MEMBERS permission.",
      };
  ...
  async removeMemberRole(user_id, role_id, { reason } = {}) {
    if (!checkPermission(await this.me(), PERMISSIONS.MANAGE_ROLES))
      throw {
        status: 403,
  ...
  async fetchAuditLogs({ limit, type, user_id, before }) {
    if (!checkPermission(await this.me(), PERMISSIONS.VIEW_AUDIT_LOG))
      throw {
        status: 403,
        error: "The bot does not have the VIEW_AUDIT_LOG permission.",
      };
   ...
   */
  async fetchInvites() {
    if (!checkPermission(await this.me(), PERMISSIONS.MANAGE_GUILD))
      throw {
        status: 403,
  ...
  calculateMessageCacheCount() {
    const x =
      (this.member_count < 500000 ? this.member_count : 499999) / 500000;
