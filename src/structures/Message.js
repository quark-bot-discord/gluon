...
class Message {
  ...
  constructor(
    ...
    if (nocache == false && this.client.cacheMessages == true) {
      ...
      if (!this.channel)
        this.client.emit(
          "debug",
          `${this.guild?.id?.toString() || this.guild_id?.toString()} NO CHANNEL`,
        );
    }
  }
  ...
  shelf() {
    ...
    const key = hash
      .sha512()
      .update(
        `${this.guild ? this.guild.id : this.guild_id}_${this.channel ? this.channel.id : this.channel_id}_${this.id}`,
      )
      .digest("hex");
