...
const getMemberAvatar = require("../../../src/util/image/getMemberAvatar");
...
describe("GetMemberAvatar", function () {
    ...
    context("check invalid input", function () {
        it("should throw an error if no member id is provided", function () {
            expect(() => getMemberAvatar(undefined, TEST_DATA.GUILD_ID, "hash")).to.throw(
                TypeError,
                "GLUON: Member id must be provided."
            );
        });
        it("should throw an error if no guild id is provided", function () {
            expect(() => getMemberAvatar(TEST_DATA.MEMBER_ID, undefined, "hash")).to.throw(
                TypeError,
                "GLUON: Guild id must be provided."
            );
        });
        it("should throw an error if the hash is not a string and not null", function () {
            expect(() => getMemberAvatar(TEST_DATA.MEMBER_ID, TEST_DATA.GUILD_ID, 123)).to.throw(
                TypeError,
                "GLUON: Member avatar hash must be a string."
            );
        });
    });
    ...
    context("check valid output", function () {
        it("should return a string", function () {
            expect(getMemberAvatar(TEST_DATA.MEMBER_ID, TEST_DATA.GUILD_ID, "hash")).to.be.a("string");
        });
        it("should return null if no hash is provided", function () {
            expect(getMemberAvatar(TEST_DATA.MEMBER_ID, TEST_DATA.GUILD_ID)).to.be.null;
        });
        it("should return the correct avatar url", function () {
            expect(getMemberAvatar(TEST_DATA.MEMBER_ID, TEST_DATA.GUILD_ID, "hash")).to.equal(
        ...
        it("should return the correct avatar url for a gif", function () {
            expect(getMemberAvatar(TEST_DATA.MEMBER_ID, TEST_DATA.GUILD_ID, "a_hash")).to.equal(
