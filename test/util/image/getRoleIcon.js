...
const getRoleIcon = require("../../../src/util/image/getRoleIcon");
...
describe("GetRoleIcon", function () {
  ...
  context("check invalid input", function () {
    ...
    it("should throw an error if the hash is not a string and not null", function () {
      expect(() =>
        getRoleIcon(TEST_DATA.ROLE_ID, 123)
      ).to.throw(TypeError, "GLUON: Role icon hash must be a string.");
    });
  });
  ...
  context("check valid output", function () {
    it("should return a string", function () {
      expect(
        getRoleIcon(TEST_DATA.ROLE_ID, "hash")
      ).to.be.a("string");
    });
    ...
    it("should return the correct icon url for a gif", function () {
      expect(
        getRoleIcon(TEST_DATA.GUILD_ID, "a_hash")
      ).to.equal(
