...
describe("DecryptMessage", function () {
    ...
    context("check invalid input", function () {
        it("should throw an error if no client is provided", function () {
            expect(() => decryptMessage(undefined, "test", TEST_DATA.MESSAGE_ID, TEST_DATA.CHANNEL_ID, TEST_DATA.GUILD_ID)).to.throw(TypeError, "GLUON: Client must be provided.");
        });
        it("should throw an error if no text is provided", function () {
            expect(() => decryptMessage({}, undefined, TEST_DATA.MESSAGE_ID, TEST_DATA.CHANNEL_ID, TEST_DATA.GUILD_ID)).to.throw(TypeError, "GLUON: Encrypted message must be provided.");
        });
    });
